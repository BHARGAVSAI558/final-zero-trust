...
app = FastAPI(title="Zero Trust Security Platform - LOCAL")
...
def calculate_risk_score(username, db):
    ...
    cursor.execute("""
        SELECT COUNT(*) as count FROM login_logs 
        WHERE user_id=%s AND success=0 AND login_time > DATE_SUB(NOW(), INTERVAL 1 HOUR)
    """, (username,))
    ...
    cursor.execute("""
        ...
        WHERE user_id=%s AND login_time > DATE_SUB(NOW(), INTERVAL 1 HOUR)
    """, (username,))
    ...
    # 6. Excessive file access
    cursor.execute("""
        SELECT COUNT(*) as count FROM file_access_logs 
        WHERE user_id=%s AND access_time > DATE_SUB(NOW(), INTERVAL 24 HOUR)
    """, (username,))
    ...
    cursor.execute("""
        SELECT COUNT(*) as count FROM file_access_logs 
        WHERE user_id=%s AND action='DELETE' AND access_time > DATE_SUB(NOW(), INTERVAL 24 HOUR)
    """, (username,))
...
@app.get("/health")
def health_check():
    return {"status": "healthy", "service": "Zero Trust Platform - LOCAL"}
...
if __name__ == "__main__":
    import uvicorn
    print("🛡️ Zero Trust System - LOCAL MODE")
